...
Negative means that expression is dropping from as the fissure goes from OF to OC This test is designed to identify which genes are changing between the OF and OC in the "during" stage.
## Volcano

```{r, fig.width=4, fig.height=2}
volcano_maker(top.table_During, title = 'During: OF vs OC',
              
              gene_list = c(top.table_During %>% filter(logFC > 0) %>% head(7) %>% row.names(),
                            top.table_During %>% filter(logFC < 0) %>% head(7) %>% row.names()))
```
