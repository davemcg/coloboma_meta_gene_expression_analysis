...
box_marker <- function(table, genes, section = c('OF','Retina')){
  qsmooth_counts %>%
...
sample_meta_D %>% 
  mutate(Section2 = case_when(Section == 'OF' ~ 'OF',TRUE ~ 'Retina')) %>%
...
- During vs Before (OF)
- During vs Before (Retina (all not OF))
- During vs After (OF)
- During vs After (Retina (all not OF))
...
```
### Expression of top 4 genes across species (OF only samples)
...
box_marker(top.table_OF_AD, 
           genes = top.table_OF_AD %>% 
             as_tibble(rownames = 'Gene') %>% 
             filter(adj.P.Val < 0.05, !grepl('HMG|H2A|UBB', Gene)) %>% pull(Gene) %>% 
             head(7),
           section = 'OF')
```

## Enrichment of OF genes (During -> After in OF)
If we *subtract* diff genes that are diff in *both*, we should enrich for genes specific to the fissure process (and not just part of the retina differentatiion process).

As we have a LOT of diff genes in the During -> After fissure test this may be helpful

### Volcano
```{r, fig.width=6, fig.height=4}
volcano_maker(top.table_OF_AD, 
              gene_list = top.table_OF_AD %>% 
                as_tibble(rownames = 'Gene') %>% 
                filter(adj.P.Val < 0.05) %>%
                filter(!Gene %in% row.names(top.table_RETINA_AD %>% 
                                              filter(adj.P.Val < 0.05))) %>% 
                head(20) %>% 
                pull(Gene))
```
...
`r top.table_OF_AD %>% 
                ...
                filter(adj.P.Val < 0.05) %>%
                filter(!Gene %in% row.names(top.table_RETINA_AD %>% 
                                              filter(adj.P.Val < 0.05))) %>% nrow()` genes are unique to the OF process (in other words don't pass significance in the retina samples at the same stage). 
```{r}
top.table_OF_AD %>% 
                as_tibble(rownames = 'Gene') %>% 
                filter(adj.P.Val < 0.05) %>%
                filter(!Gene %in% row.names(top.table_RETINA_AD %>% 
                                              filter(adj.P.Val < 0.05))) %>% DT::datatable()
```
...
## GO Enrichment of OF sections, from During Fusion to After
```{r}
library(clusterProfiler)
library(enrichplot)


diff_genes <- top.table_OF_AD %>% as_tibble(rownames = 'Gene') %>% filter(adj.P.Val < 0.05)
...
egoOF_AD <- enrichGO(gene          = eg_diff_genes$ENTREZID,
                  universe      = eg_universe$ENTREZID,
                  OrgDb         = org.Hs.eg.db,
                  ont           = "all",
                  readable      = TRUE)

diff_genes <- top.table_RETINA_AD %>% as_tibble(rownames = 'Gene') %>% filter(adj.P.Val < 0.05)
eg_diff_genes <- bitr(diff_genes$Gene, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
eg_diff_genes <- diff_genes %>% left_join(., eg_diff_genes, by = c('Gene' = 'SYMBOL'))
eg_universe = bitr(top.table_RETINA_AD %>% as_tibble(rownames = 'Gene') %>% pull(Gene), fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")


egoRETINA_AD <- enrichGO(gene          = eg_diff_genes$ENTREZID,
                      universe      = eg_universe$ENTREZID,
                      OrgDb         = org.Hs.eg.db,
                      ont           = "all",
                      readable      = TRUE)
...
p1 <- dotplot(egoOF_AD, showCategory=30) + ggtitle("Dotplot for GO, OF")
p1
...
cnet <- cnetplot(egoOF_AD, foldChange = geneList) + scale_color_viridis_c(name = 'log2(FoldChange)')
cnet
```
